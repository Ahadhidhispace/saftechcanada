<script context="module">
	...
	const activeLocationId = writable(0);
...
<script>
	...
	const selectionItemId = location_id++;
	$: active = $activeLocationId === selectionItemId;

	/**
	 * @param {{ target: { dataset: string; }; }} event
	 */
	// function setActive(event){
	//     $activeLocationId = selectionItemId
...
<button
	on:click={(event) => {
		...
		if (active) {
			$activeLocationId = selectionItemId;
			$selectedLocation = JSON.stringify(event.currentTarget.getAttribute('data-location'));
			$page.url.searchParams.delete('location'); // Remove all the previous "location"
			$page.url.searchParams.append('location', $activeLocationId.toString()); // Then add new one
			console.log($page);
		} else {
			$activeLocationId = -1;
			$selectedLocation = '';
