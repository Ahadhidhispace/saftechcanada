...
<div class="relative max-h-full transition-all duration-500 ease-out">
	<textarea
		bind:value={inputValue}
		class="peer mt-0 block w-full rounded-lg bg-saftech-white focus:ring-0 px-4 pr-5 py-4 text-base font-bold leading-tight text-saftech-black/70 placeholder:text-saftech-black/80 placeholder:font-bold placeholder:mt-20 focus:placeholder:opacity-10 border-none focus:border-none pb-[0.600rem] focus:pb-[0.625rem] pt-[1.300rem] focus:pt-[1.625rem] focus:text-saftech-black/70 {error
			? 'focus:outline-red-600'
			: 'focus:outline-normal-blue'} min-h-[140px] focus:outline-4 focus:border-8 focus:outline-offset-3 outline-none peer-focus:text-normal-blue transition-all duration-500 ease-out"
		id="floatingInputAchievement"
		
		placeholder={error ? placeholder : guidePlaceholder}
	/>
	<label
		for="floatingInputAchievement"
	...
	>
	<svg
		class=" hidden absolute top-[10%] left-[90%] fill-saftech-black/80 {error
