<script>
	...
	let whiteBackgroundOptimized = false //JSON.parse($page.data.optimize) // JSON.parse($page.url.searchParams.get('nav_background_optimize')) || false
	...
	async function handleLogout(event) {
		...
		if (result.type === 'error' || result.type === 'failure'){
					...
					$openAlert.trableshoot = 'There was an error logging you out. Make sure you have a working internet connection'
	...
	// 	// reload when supabasea auth state changes
	// 	const {
	// 		data: { subscription }
					...
					>
						...
						<div class="relative ml-3">
							...
							{#if viewingProfile}
								...
								<div
									class="z-50 absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
									role="menu"
				...
				<div class="h-full">
					<ul class="h-full flex space-x-10">
						{#each Object.keys(links) as navItem, index}
							...
							id="" class="relative font-bold text-lg text-saftech-white before:rounded-lg { !showingSubNav ? 'before:opacity-0 before:h-[50px] ':'before:opacity-100 before:h-[450px]'}  before:absolute before:-inset-2 before:bg-gradient-to-b  { navItem ===  selectedFeatureId ? 'before:from-normal-blue/10 before:via-normal-blue/50 before:to-normal-blue/50 px-4':' px-0 before:from-transparent before:via-transparent before:to-transparent'} transition-all duration-500 ease-out">{ Object.entries(links)[index][1].displayText }</li> 
						{/each}
					</ul>
				</div>	
			</section>
				...
				>
					...
					<div  class=" z-50 relative ml-3">
						...
						{#if viewingProfile}
							<!-- and only if the user is logged in -->
							<div
							transition:scale={{duration: 500, delay: 0, easing: quintInOut}}
							
								class=" z-50 absolute right-0 { viewingProfile ? 'mt-6':'mt-2'} w-48 origin-top-right drop-shadow-2xl rounded-md bg-gradient-to-tr from-saftech-white to-white py-0 shadow-2xl ring-1 ring-saftech-white ring-opacity-5 focus:outline-none transition-all delay-700 duration-500 ease-[cubic-bezier(0,1.3,.19,.94)]"
								role="menu"
								aria-orientation="vertical"
								aria-labelledby="user-menu-button"
								tabindex="-1"
							...
							<div class="flex flex-col items-center p-4">
								...
								<!-- Active: "bg-gray-100", Not Active: "" -->
								<h2 class="font-normal text-xl text-saftech-black/80 mt-3">Welcome</h2>
								<a
									href="/user/profile"
									class=" relative after:absolute after:w-10 after:h-full after:bg-gradient-to-l after:from-[rgb(251,251,251)] after:to-transparent after:right-0 after:top-1/2 after:-translate-y-1/2 text-left text-xs text-normal-blue max-w-[85%] line-clamp-2" 
									role="menuitem"
									tabindex="-1"
									disabled="true"
									id="user-menu-item-0">{data.session.user.email}</a
								>
							</div>
							<div class="flex justify-between items-center bg-saftech-gray/10 rounded-lg">
								<a
									href="/user/settings"
									class="block px-4 py-2 text-sm text-gray-700"
									role="menuitem"
